...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return { title: `Bài khóa: ${lesson.title} - Chinese Learning App` };
  } catch (error) {
    return { title: "Bài khóa không tồn tại" };
  }
}
...
export default async function DialoguePage({
  ...
  const { lesson, dialogueSentences } = lessonData;

  const steps = [
    { label: "Bài khóa", bg: "#C1272D" },
    { label: "Từ vựng", bg: "#EADFC6" },
      ...
      <div className="w-full max-w-[660px] px-5">
        ...
        <div className="sticky top-0 z-20 pt-5 pb-3.5 bg-gradient-to-b from-[#FBF6EC] to-[#FBF6EC]/0 dark:from-background dark:to-transparent">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="text-[13px] font-semibold text-[#9A8F7E] dark:text-white/40">20%</div>
          </div>
        ...
        <div className="relative mt-1.5 mb-[22px] bg-[#FFFDF8] dark:bg-[#FFFDF8]/5 border border-[#EFE4CE] dark:border-white/10 rounded-[24px] px-[26px] pt-[26px] pb-[24px] shadow-[0_10px_30px_rgba(120,90,40,0.06)] dark:shadow-none overflow-hidden animate-fade-in-up">
          ...
          <div className="text-[13px] font-bold tracking-[1.5px] uppercase text-[#C1272D] mb-2">Bài khóa</div>
          ...
          <div className="flex items-center gap-3.5 mt-[18px] pt-4 border-t border-dashed border-[#EADEC4] dark:border-white/10 flex-wrap">
            ...
            <div className="ml-auto text-[13px] text-[#A89C88] font-semibold">{dialogueSentences.length} câu</div>
          </div>
        </div>
        ...
        <div className="text-center mt-3 text-[13px] text-[#A89C88] font-semibold">Tiếp theo: Từ vựng</div>
