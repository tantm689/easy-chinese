...
import { getVocabExercises, getLessonDetail } from "@/lib/queries";
import VocabExerciseClient from "./VocabExerciseClient";
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return { title: `Bài tập: ${lesson.title} - Chinese Learning App` };
  } catch (error) {
...
export default async function VocabExercisePage({
  ...
  try {
    ...
    exercises = await getVocabExercises(id);
  } catch (error) {
    notFound();
  }
  ...
  const steps = [
    ...
    { label: "Luyện từ", bg: "#C1272D" },     // Current
    ...
    <div className="min-h-screen pb-12 font-sans bg-[#FBF6EC] dark:bg-background text-[#2B2622] dark:text-foreground flex justify-center">
      <div className="w-full max-w-[660px] px-5">
        ...
        <div className="sticky top-0 z-20 pt-5 pb-3.5 bg-gradient-to-b from-[#FBF6EC] to-[#FBF6EC]/0 dark:from-background dark:to-transparent">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2.5">
              ...
              <div className="font-bold text-[15px] tracking-wide">Bước 3/5 · Luyện từ</div>
            </div>
            <div className="text-[13px] font-semibold text-[#9A8F7E] dark:text-white/40">60%</div>
          </div>
        ...
        <div className="relative mt-1.5 mb-[22px] bg-[#FFFDF8] dark:bg-[#FFFDF8]/5 border border-[#EFE4CE] dark:border-white/10 rounded-[24px] px-[26px] pt-[26px] pb-[24px] shadow-[0_10px_30px_rgba(120,90,40,0.06)] dark:shadow-none overflow-hidden animate-fade-in-up">
          ...
          <div className="text-[13px] font-bold tracking-[1.5px] uppercase text-[#C1272D] mb-2">Bài tập trắc nghiệm</div>
          <div className="font-serif text-[30px] font-bold tracking-wide mb-1.5 leading-tight">Luyện tập từ vựng</div>
