...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return { title: `Từ vựng: ${lesson.title} - Chinese Learning App` };
  } catch (error) {
    return { title: "Từ vựng không tồn tại" };
  }
}
...
export default async function VocabularyPage({
  ...
  const { lesson, vocabulary } = lessonData;

  const steps = [
    { label: "Bài khóa", bg: "#C1272D" },     // Done
    { label: "Từ vựng", bg: "#C1272D" },      // Current
    { label: "Luyện từ", bg: "#EADFC6" },
    { label: "Câu mẫu", bg: "#EADFC6" },
    ...
    <div className="min-h-screen pb-12 font-sans bg-[#FBF6EC] dark:bg-background text-[#2B2622] dark:text-foreground flex justify-center">
      <div className="w-full max-w-[660px] px-5">
        ...
        <div className="sticky top-0 z-20 pt-5 pb-3.5 bg-gradient-to-b from-[#FBF6EC] to-[#FBF6EC]/0 dark:from-background dark:to-transparent">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2.5">
              <Link href={`/lessons/${id}/dialogue`} className="text-foreground/70 hover:text-[#C1272D] transition-colors p-2 -ml-2 rounded-full hover:bg-[#C1272D]/10 flex-shrink-0">
              ...
              <div className="font-bold text-[15px] tracking-wide">Bước 2/5 · Từ vựng</div>
            </div>
            <div className="text-[13px] font-semibold text-[#9A8F7E] dark:text-white/40">40%</div>
          </div>
        ...
        <div className="relative mt-1.5 mb-[22px] bg-[#FFFDF8] dark:bg-[#FFFDF8]/5 border border-[#EFE4CE] dark:border-white/10 rounded-[24px] px-[26px] pt-[26px] pb-[24px] shadow-[0_10px_30px_rgba(120,90,40,0.06)] dark:shadow-none overflow-hidden animate-fade-in-up">
          ...
          <div className="text-[13px] font-bold tracking-[1.5px] uppercase text-[#C1272D] mb-2">Từ vựng mới</div>
          ...
          <div className="flex items-center gap-3.5 mt-[18px] pt-4 border-t border-dashed border-[#EADEC4] dark:border-white/10 flex-wrap">
            <div className="text-[13px] font-semibold text-[#8A8071]">
              Hãy nghe và đọc theo các từ vựng dưới đây
            </div>
            <div className="ml-auto text-[13px] text-[#A89C88] font-semibold">{vocabulary.length} từ</div>
          </div>
        </div>
        ...
        {/* Next button */}
        <Link 
          href={`/lessons/${id}/vocab-exercise`}
        ...
        <div className="text-center mt-3 text-[13px] text-[#A89C88] font-semibold">Tiếp theo: Luyện từ</div>
